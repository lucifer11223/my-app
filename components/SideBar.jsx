        ...
        <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
            <div>
                <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
                    < Image className={expand ? 'w-36' : 'w-12'} src={expand ? assets.logo_text : assets.logo_icon} alt='' />
                    ...
                    <div onClick={() => expand ? setExpand(false) : setExpand(true)}
                        ...
                        <div className={`absolute w-max ${expand ? 'left-1/2 top-12 -translate-x-1/2 ' : '-top-12 left-0'} opacity-0 group-hover:opacity-100 transition bg-black duration-300 text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none`}>
                ...
                ${expand ? "bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max" : "group  relative w-9 h-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                ...
                <div className={`mt-8 text-white/25 text-sm ${expand ? 'block' : 'hidden'}`}>
                    ...
                    {chats.map((chat) => <ChatLabel openMenu={openMenu} setOpenMenu={setOpenMenu} key={chat._id} id={chat._id} name={chat.name} />)}
                </div>
            </div>
            ...
            <div>
                <div className={`flex items-center cursor-pointer group relative ${expand ?
                    ...
                    group-hover:opacity-100 group-hover:block hidden transition`}>
                        <div className='relative w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg'>
                            <Image src={assets.qrcode} alt='' className='w-44' />
                            <p>Scan to get DeepSeek Ap </p>
                            <div className={`w-3 h-3 absolute bg-black rotate-45 ${expand ?
                                'right-1/2' : 'left-4'
                                } -bottom-1.5`}></div>
                        </div>
                    ...
                    {expand && <>
                        <span>Get App</span>
                        <Image src={assets.new_icon} alt='' className='' />
                    </>}
                </div>
